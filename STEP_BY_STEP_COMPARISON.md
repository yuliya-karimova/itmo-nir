...
4. Заполните поля:
   - **Имя заголовка**: `x-user-variant` (стандартный заголовок для A/B теста)
   - **Значение для сравнения**: `A` (для варианта A) или `B` (для варианта B)
   - Примечание: можно использовать любое значение, например `x-user-variant-a` для варианта A
...
**Вариант 1: Через модификатор заголовков в браузере (рекомендуется)**

1. Установите расширение для модификации заголовков:
   - Chrome: "ModHeader" или "Requestly"
   - Firefox: "Modify Headers"
2. Добавьте заголовок:
   - Имя: `x-user-variant`
   - Значение: `A` или `B` (или любое другое значение, которое вы указали в condition блоке)
...
**Вариант 3: На Railway (для продакшена)**

При тестировании на Railway используйте тот же заголовок:
```bash
...
1. **Добавляем endpoint на бэкенде** (`classic/backend/server.js`):
   - Endpoint `/api/banner-variant` проверяет заголовок `x-user-variant`
   - Возвращает вариант A или B в зависимости от заголовка

2. **Обновляем фронтенд** (`classic/frontend/src/pages/HomePage.js`):
   - Заменяем захардкоженный баннер на запрос к `/api/banner-variant`
   - Передаем заголовок `x-user-variant` через axios
...
4. **Перезапускаем бэкенд (если нужно):**
```bash
cd classic
docker compose restart backend
```

#### Шаг 3.6: Тестирование A/B теста

**Вариант 1: Через localStorage (для быстрого тестирования)**

1. Откройте консоль браузера (F12)
2. Выполните:
   ```javascript
   localStorage.setItem('x-user-variant', 'A'); // или 'B'
   location.reload();
   ```

**Вариант 2: Через модификатор заголовков**

1. Установите расширение "ModHeader" для Chrome
2. Добавьте заголовок:
   - Имя: `x-user-variant`
   - Значение: `A` или `B`
3. Откройте страницу - должен показаться соответствующий баннер

**Вариант 3: Через curl**

```bash
# Вариант A
curl http://localhost:3011/api/banner-variant -H "x-user-variant: A"
...
### Шаг 3.8: Оценка поддерживаемости

**BDUI:**
- Изменения через админку (condition блок)
- Не требует пересборки фронтенда
- Не требует изменения кода
- Легко откатить (удалить или изменить condition блок в админке)
- Можно изменить условие или варианты контента без деплоя
- **Оценка: 5/5 баллов**

**Classic:**
- Требуется изменение кода компонента (HomePage.js)
- Требуется добавление endpoint на бэкенде
- Пересборка фронтенда обязательна
- Изменение вариантов требует изменения кода и пересборки
- **Оценка: 2-3/5 баллов**
...
1. **Создан Service Worker** в `bdui/frontend/public/sw.js`:
   - Кэширует API запросы к `/api/page/` для работы в оффлайн режиме
   - Использует стратегию "Cache First" для статических ресурсов
   - Автоматически обновляет кэш в фоне при наличии сети

2. **Зарегистрирован Service Worker** в `bdui/frontend/src/index.js`:
   - Регистрация происходит при загрузке приложения
   - Работает только в браузерах с поддержкой Service Workers

**Что делает Service Worker:**
- Кэширует ответы API `/api/page/` для использования в оффлайн режиме
- При наличии сети обновляет кэш в фоне
- При отсутствии сети возвращает данные из кэша
- Кэширует статические ресурсы (CSS, JS) для быстрой загрузки

**Проверка работы:**
- Service Worker должен зарегистрироваться в консоли браузера (сообщение "Service Worker registered successfully")
- Можно проверить в Chrome DevTools → Application → Service Workers
- Убедитесь, что Service Worker активен (статус "activated and is running")
- Проверьте, что файл `/sw.js` доступен (откройте `http://localhost:3000/sw.js` в браузере)

**Важно:**
- Service Worker работает на localhost без HTTPS (это исключение)
- На production нужен HTTPS для работы Service Worker
- После изменений в Service Worker может потребоваться обновление кэша (Unregister в DevTools)

#### Шаг 4.2: Тестирование оффлайн

**Важно:** Сначала нужно посетить страницу с сетью, чтобы она закэшировалась!

**Шаг 1: Кэширование страницы (с сетью)**
```bash
# 1. Открываем страницу с сетью
open http://localhost:3000

# 2. Дожидаемся полной загрузки страницы
# 3. Открываем Chrome DevTools → Console
# 4. Проверяем, что есть сообщения "Service Worker: Cached /"
# 5. Открываем Chrome DevTools → Application → Cache Storage
# 6. Проверяем, что в кэше "bdui-cache-v1" есть файлы (index.html, JS, CSS)
```

**Шаг 2: Тестирование оффлайн**
```bash
# 1. Открываем Chrome DevTools → Application → Service workers
# 2. Ставим галочку "Offline" (это включает оффлайн режим)
# 3. Обновляем страницу (F5 или Ctrl+R)
# 4. Проверяем результат
```

**Ожидаемый результат**: 
- ✅ Страница загружается из кэша
- ✅ В консоли видны сообщения "Service Worker: Found in cache"
- ✅ Страница отображается корректно, данные берутся из кэша

**Если не работает:**
- Проверьте, что в Cache Storage есть закэшированные файлы
- Проверьте консоль на наличие ошибок
- Убедитесь, что Service Worker активен (зеленый статус "activated and is running")
- Попробуйте "Unregister" и перезагрузите страницу с сетью

---

### Классический подход

#### Шаг 4.3: Подготовка кэширования

1. **Создан Service Worker** в `classic/frontend/public/sw.js`:
   - Кэширует API запросы к `/api/features` и `/api/banner-variant`
   - Использует стратегию "Cache First" для статических ресурсов
   - Автоматически обновляет кэш в фоне при наличии сети

2. **Зарегистрирован Service Worker** в `classic/frontend/src/index.js`:
   - Регистрация происходит при загрузке приложения
   - Работает только в браузерах с поддержкой Service Workers

**Что делает Service Worker:**
- Кэширует ответы API для использования в оффлайн режиме
- При наличии сети обновляет кэш в фоне
- При отсутствии сети возвращает данные из кэша
- Кэширует статические ресурсы (CSS, JS) для быстрой загрузки
...
- Можно проверить в Chrome DevTools → Application → Service Workers
- Убедитесь, что Service Worker активен (статус "activated and is running")
- Проверьте, что файл `/sw.js` доступен (откройте `http://localhost:3010/sw.js` в браузере)
...
**Ожидаемый результат**: 
- ✅ UI уже встроен в приложение (HTML, CSS, JS)
- ✅ Запросы к API (`/api/features`, `/api/banner-variant`) работают из кэша
- ✅ Service Worker кэширует данные
- ✅ Страница работает в оффлайн режиме
...
| Аспект | BDUI | Classic |
|--------|------|---------|
| Работает оффлайн? | Да (с Service Worker и кэшем) | Да (с Service Worker и кэшем) |
| UI доступен оффлайн? | Да (кэшируется) | Да (встроен в приложение) |
| Данные доступны оффлайн? | Да (кэшируются через SW) | Да (кэшируются через SW) |
| Требуется Service Worker? | Да | Да (для данных) |
| Размер кэша | Больше (UI + данные) | Меньше (только данные, UI встроен) |

**Результаты тестирования:**

**BDUI:**
- ✅ Работает в оффлайн режиме
- ✅ Service Worker кэширует API запросы
- ✅ Страница загружается из кэша
- ✅ Данные доступны из кэша

**Classic:**
- ✅ Работает в оффлайн режиме (с Service Worker)
- ✅ Service Worker кэширует API запросы
- ✅ UI встроен в приложение (не требует кэширования)
- ✅ Данные доступны из кэша

**Оценка**: 
- BDUI: 5/5 баллов (полная поддержка оффлайн)
- Classic: 5/5 баллов (полная поддержка оффлайн)

**Преимущества:**
- **BDUI**: Единый механизм кэширования (UI + данные)
- **Classic**: UI встроен в приложение (меньше размер кэша, быстрее первая загрузка)
...
✅ **Выполнено:** Создан файл `comparison_results/final_comparison.md` со сводной таблицей всех результатов.

**Содержимое:**
- Сравнение по 6 параметрам (TTM, Трафик, Производительность, Ресурсы, Оффлайн, Поддержка)
- Детальный анализ по каждому сценарию
- Итоговые оценки: BDUI 26/30, Classic 25/30
- Рекомендации по применению каждого подхода
...
✅ **Выполнено:** Создан скрипт `comparison_results/visualize.py` и сгенерированы графики.
...
- [x] Написаны выводы и рекомендации (в `final_comparison.md`)
- [x] Подготовлен отчет для НИР3 (`final_comparison.md`)
- [x] Развернуто на облаке (Railway) для более реалистичного сравнения TTM
